...
void HAL::serialPackData(
  const OSDK_Uart_Header_t & header, const uint8_t cmd_set, const uint8_t cmd_id, const void * data,
  ...
  std::unique_lock<std::mutex> & lock, const uint8_t cmd_set, const uint8_t cmd_id)
{
  ...
  if (!serial_rx_data_cond.wait_for(
        lock, timeout, std::bind(&DIABLO::OSDK::HAL::verifyRXType, this, cmd_set, cmd_id)))
    return NULL;

  *(reinterpret_cast<uint8_t *>(rx_data) - 2) = *(reinterpret_cast<uint8_t *>(rx_data) - 1) =
    -1;  // mark rx cmd set and id to be invalid
  return getRXData();
}

uint8_t HAL::serialSend_ack(
  ...
  const void * data, const uint32_t data_len)
{
  ...
  static const chrono::duration<int, milli> timeout(100);
  {
    ...
    if (!serial_rx_ack_cond.wait_for(
          lock, timeout, std::bind(&DIABLO::OSDK::HAL::verifyRXType, this, cmd_set, cmd_id))) {
  ...
  while (true) {
    {
      ...
      if (serial_tx_idle && !serial_tx_cond.wait_for(lock, timeout, [this](void) {
            return !serial_tx_idle;
          })) {
        serial_tx_idle = true;
        continue;
      }
...
uint8_t HAL_Pi::init(const std::string dev, const int baud)
{
  ...
  rx_data = reinterpret_cast<void *>(serial_rxbuf + 2 + sizeof(OSDK_Uart_Header_t));
  return 0;
}

uint8_t HAL_Pi::serialSend(
  const OSDK_Uart_Header_t & header, const uint8_t cmd_set, const uint8_t cmd_id, const void * data,
  const uint32_t data_len)
{
  static const chrono::duration<int, milli> timeout(100);
  ...
  serial_tx_duration =
    size * 10. / serial_br + 2e-4;  // time needed to finish transmitting. unit: second
  serial_seq++;
  serial_tx_idle = false;
  serial_tx_cond.notify_all();

  for (uint8_t i = 0; i < size; i++) mySerial.WriteChar(serial_txbuf[i]);
...
void HAL_Pi::RXMonitorProcess(void)
{
  ...
  while (true) {
    ...
    while (static_cast<int>(mySerial.Available()) < static_cast<int>(data_len)) usleep(byte_micro);
