...
uint8_t Virtual_RC::obtain_control(uint16_t timeout_ms)
{
  ...
  if (
    ...
    ((1 << (vehicle->telemetry->id * 2)) | (1 << (4 + vehicle->telemetry->id * 2))))
    exit(0);  // control is taken by Movement Ctrl

  Header header;
  header.data.LEN = sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Virtual_RC_Request_t) + OSDK_MISC_SIZE;
  ...
  while (ack != 0x0002) {
    uint8_t result = vehicle->hal->serialSend_ack(
      header.data, ack, OSDK_VIRTUAL_RC_SET, OSDK_VIRTUAL_RC_AUTHORIZE_ID, &req,
      sizeof(OSDK_Virtual_RC_Request_t));

    if (result) return result;
    if (ack == 0x0000) {
      std::cerr << "SDK control disabled on manual RC controller, check your RC setting"
                << std::endl;
      return 3;
    }
    if (ack == 0x000A) {
...
uint8_t Virtual_RC::release_control()
{
  ...
  vehicle->hal->serialSend(
    header.data, OSDK_VIRTUAL_RC_SET, OSDK_VIRTUAL_RC_AUTHORIZE_ID, &req,
    sizeof(OSDK_Virtual_RC_Request_t));
...
>>>>>>> 67a684b70db424c49caea5813a47e0e0359b4918
  ...
  Header header;
  header.data.LEN = sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Virtual_RC_t) + OSDK_MISC_SIZE;
  header.data.SEQ = vehicle->hal->serial_getSeq();
  header.append_crc();

  return vehicle->hal->serialSend(
    header.data, OSDK_VIRTUAL_RC_SET, OSDK_VIRTUAL_RC_DATA_ID, &data, sizeof(OSDK_Virtual_RC_t));
}
